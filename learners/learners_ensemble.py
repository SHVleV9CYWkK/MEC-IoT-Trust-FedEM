    ...
    def evaluate_iterator(self, iterator):
        ...
        with torch.no_grad():
            for (x, y, _) in iterator:
                ...
                metric = self.metric(y_pred, y).mean().item()
                global_metric += metric

            result = global_metric / (len(iterator) if self.is_binary_classification else n_samples)
            return global_loss / n_samples, result
